...
| `ray-argv-shell` | FAIL | `ray job submit -- <argv>` does **not** exec argv: it re-joins it with `subprocess.list2cmdline` — the *Windows* quoting function, which quotes on whitespace only — and runs the string under `/bin/sh`. A value with shell metacharacters and no spaces arrives raw, so `--rollout-stop <\|im_end\|>` parses as redirect+pipe and the job dies ~40s in, *after* ray has accepted it (jobs 68024-68026, all three arms). The same path silently eats backslashes: `self_attention\.` becomes `self_attention.`, a regex dot matching any character. Quoting cannot be judged by eye (`"${VAR:-'<\|x\|>'}"` is safe, `'<\|x\|>'` is not) and shlex round-trips *pass* on the fatal argv, so the check demands the `/bin/sh` oracle — `argv_survives_ray_shell` from `<monorepo>_rl/lib_argv_guard.sh` — or an explicit `# preflight: argv-guarded` |
